...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async fbUser => {
      if (fbUser) {
        // 🔐 Forzar refresh de claims
        await fbUser.getIdToken(true)
        const token = await fbUser.getIdTokenResult()

        console.log('🔐 CLAIMS ACTIVOS:', token.claims)
        console.log('🧪 ADMIN REAL:', token.claims.admin === true)

        const ref = doc(db, 'usuarios', fbUser.uid)
        const snap = await getDoc(ref)

        setFirebaseUser(snap.exists() ? { ...fbUser, ...snap.data() } : fbUser)
      } else {
  ...
  function getNivel() {
    return Number(adminSession?.nivel || 0)
  }
  ...
  return (
    <AuthContext.Provider
      value={{
        ...
        // 👇 internos (por si algún componente nuevo los usa)
        firebaseUser,
        adminSession,
        rolUsuario,
        permisos,
