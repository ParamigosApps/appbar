...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (topic === 'payment') {
      ...
      console.log('✅ pago actualizado', pagoId)
      ...
      if (pago.tipo === 'compra') {
        ...
        if (pago.estado !== 'aprobado') {
          console.log('ℹ️ compra no aprobada, skip', {
            pagoId,
            estado: pago.estado,
          })
        } else {
          const { marcarCompraPagadaDesdePago } = await import(
            './generarCompraPagadaMercadoPago.js'
          )

          // 🔑 FIRMA CORRECTA
          await marcarCompraPagadaDesdePago(pagoId, payment)
        }
      }
    }
    await ref.update({
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (topic === 'payment') {
      ...
      // 4️⃣ ENTRADAS
      if (pago.tipo === 'entrada') {
        console.log('🎟️ pago tipo entrada → lógica existente')
        // ⚠️ ACÁ NO TOCAMOS NADA (ya lo tenías funcionando)
      }
